...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult,BOOL parentContent)
{
  ...
  if (currentNode->name)
    {
      NSString *currentNodeContent =
        [NSString stringWithCString:(const char *)currentNode->name encoding:NSISOLatin1StringEncoding];
  ...
  if (currentNode->content && currentNode->content != (xmlChar *)-1)
    {
      NSString *currentNodeContent =
        [NSString stringWithCString:(const char *)currentNode->content encoding:NSISOLatin1StringEncoding];
      ...
      while (attribute)
        {
          ...
          NSString *attributeName =
            [NSString stringWithCString:(const char *)attribute->name encoding:NSISOLatin1StringEncoding];
          if (attributeName)
            {
...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query)
{
  ...
  xpathObj = xmlXPathEvalExpression((xmlChar *)[query cStringUsingEncoding:NSISOLatin1StringEncoding], xpathCtx);
  if(xpathObj == NULL) {
